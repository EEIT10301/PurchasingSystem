...
<body class="bg">
	<c:if test="${not empty thispro}">
		<div class="right">
			<div class='container'>
				<h3 align="center">採購單編號:${pomainlist.po_id}</h3>
				<br />
				...
				<table class="table table-striped table-hover">
					...
					<div data-toggle="collapse" data-target="#demo2"
						...
						<h5 class="text-center font-weight-bold">採購單明細 ↓</h5>
					</div>
				...
				<div class="card">
					<div data-toggle="collapse" data-target="#demo1"
						class="accordion-toggle">
						<h5 class="text-center font-weight-bold">詢價紀錄 ↓</h5>
					</div>
						...
						<table class="table table-striped table-hovertext-center">
							<c:forEach var='poquerys' varStatus='vs' items='${poquery}'>
								...
								<tr data-toggle="collapse" data-target="#demo1"
									...
									<th class="text-center">詢價時間</th>
									<th class="text-center">詢價總價</th>

								</tr>
								<tr>

									<td class="text-center">${poquerys.pO_Vendor_InfoBean.vendor_name}</td>
									<td class="text-center">${poquerys.po_querydate}</td>
									<td class="text-center">${poquerys.po_totalprice}</td>
					...
					<c:forEach var='posignpro' varStatus='vs' items='${posignprocess}'>
						<c:if test="${posignpro.sig_rank==5}">
							<c:if
								...
								<tr>
									<td>3</td>
									<td>${posignpro.employeeBean.emp_name}
										${posignpro.sig_sta}</td>
									<td>${posignpro.employeeBean.emp_dep}
										${posignpro.employeeBean.emp_job}</td>
									<c:if test="${not empty posignpro.sig_date}">
										<td>${posignpro.sig_date}</td>
				...
				<form
					...
					<p>
						<Input type='submit' name='send' value='${sendsubmit1}'> <Input
							type='submit' name='send' value='${sendsubmit2}'>
				</form>
			</div>
		</div>
	</c:if>
